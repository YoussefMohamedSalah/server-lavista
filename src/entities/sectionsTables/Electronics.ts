...
export class Electronics extends BaseEntity {
    ...
    // Relations
    // -----*-----*-----*-----*-----*-----*-----*-----*-----*-----*
    @OneToOne(() => Village, village => village.electronics_section, { onDelete: 'CASCADE' })
    village: Village;

    @OneToMany(() => ElectronicsItem, electronicsItem => electronicsItem.category, { cascade: true, onDelete: 'CASCADE' })
    items: ElectronicsItem[];
    // -----*-----*-----*-----*-----*-----*-----*-----*-----*-----*
    @Column({
        type: 'timestamp',
